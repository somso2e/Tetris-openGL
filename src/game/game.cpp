...
void Game::Init() {
    ...
    PauseButtonTextureID_ =
        Renderer::LoadTexture("res/textures/PauseButton.png");
...
void Game::Update() {
    ...
    if (State_ == GameState::Active) {
        ...
        // Move the active tetromino down if piece hasnt moved for "FallSpeed_"
        // seconds.
        ...
        if (ActiveTetromino_.placed) {
            shouldSave = true;
            action = ActiveTetromino_.rotation * NUM_OF_CELLS_W +
                     ActiveTetromino_.pos.x;
        ...
        Color heldColor = FirstSwap_ ? Color::Empty
                                     : TETROMINOES.at(HeldTetrominoType_).color;
        ...
        for (auto y = 0; y < NUM_OF_CELLS_H; y++) {
            for (auto x = 0; x < NUM_OF_CELLS_W; x++) {
                BoolDisplayMap_.at(x).at(y) =
                    (DisplayMap_.at(x).at(y) != Color::Empty &&
                     DisplayMap_.at(x).at(y) != Color::Gray);
            }
        }
    ...
    if (State_ == GameState::CatchedHotkey) {
        ...
        InvalidCustomHotkey_ = false;
        if (InputCustomHotkey_ !=
            DisplayHotkeys_.at(HighlightedSettingsInd_).Value) {
...
bool Game::HasCollied(Tetromino tetromino) {
    for (auto const &piece : tetromino.blocks) {
        ...
        // or if the cell in the map is a color other than gray AKA it's
        // occupied
        if ((blocks.x) >= NUM_OF_CELLS_W || (blocks.y) >= NUM_OF_CELLS_H ||
...
bool Game::MoveTetromino(Tetromino &tetromino, Movement movement) {
    // Offsets applied when rotating tetrominoes based on the official "Super
    // Rotation System" Offsets for J, L, S, T and Z Tetrominoes
    static const std::vector<std::array<point, 5>> OFFSETS_JLSTZ{{
        {point(0, 0), point(0, 0), point(0, 0), point(0, 0),
         point(0, 0)}, // Rotation state 0
        {point(0, 0), point(+1, 0), point(+1, +1), point(0, -2),
         point(+1, -2)}, // Rotation state R
        {point(0, 0), point(0, 0), point(0, 0), point(0, 0),
         point(0, 0)}, // Rotation state 2
        {point(0, 0), point(-1, 0), point(-1, 1), point(0, -2),
         point(-1, -2)} // Rotation state L
    }};
    // Offsets for I Tetromino
    static const std::vector<std::array<point, 5>> OFFSETS_I{{
        {point(0, 0), point(-1, 0), point(+2, 0), point(-1, 0),
         point(+2, 0)}, // Rotation state 0
        {point(-1, 0), point(0, 0), point(0, 0), point(0, -1),
         point(0, +2)}, // Rotation state R
        {point(-1, -1), point(+1, -1), point(-2, -1), point(+1, 0),
         point(-2, 0)}, // Rotation state 2
        {point(0, -1), point(0, -1), point(0, -1), point(0, -1),
         point(0, +2)} // Rotation state L
    }};
    ...
    default:
        if (tetromino.type != O) {
            ...
            for (int i = 0; i < 5; i++) {
                auto offset = offsetArray.at(original.rotation).at(i) -
                              offsetArray.at(tetromino.rotation).at(i);
...
std::string Game::GetTime(double elapsedTime) {
    ...
    int millis = t * 1000;
    char str[10]{};
    std::snprintf(str, 10, "%02d:%02d:%03d", min, sec, millis);
...
void Game::UpdateHighScore() {
    ...
    if (GameMode_ == GameMode::Classic) {
        ...
        auto it =
            std::upper_bound(records.begin(), records.end(), Score_,
        ...
        auto it =
            std::upper_bound(records.begin(), records.end(), ElapsedTime_,
                             [](auto &lhs, auto &rhs) { return lhs < rhs; });
